...
class mdDateTimePicker {
  ...
  constructor({
    ...
    if (
      ...
      !document.getElementById(`mddtp-picker__${this._type}`)
    ) {
      this._buildDialog();
    }
  }
  ...
  _selectDialog() {
    ...
    this._sDialog.picker = document.getElementById(
      `mddtp-picker__${[this._type]}`
    );
    /**
    ...
    while (i--) {
      this._sDialog[sDialogEls[i]] = document.getElementById(
        `mddtp-${this._type}__${sDialogEls[i]}`
      );
    }
  ...
  _hideDialog() {
    ...
    // reset classes
    if (this._type === "date") {
      years.classList.remove(
        this.displayClasses.show,
        this.displayClasses.hide
      );
  ...
  _initMonth(view, m) {
    ...
    const firstDayOfMonth = moment
      .weekdays(true)
      .indexOf(moment.weekdays(false, moment(m).date(1).day()));
    /*
    ...
    const lastDayOfMonth =
      parseInt(moment(m).endOf("month").format("D"), 10) + firstDayOfMonth - 1;
  ...
  _addCellClickEvent(el) {
    ...
    el.onclick = function (e) {
      if (
        ...
        e.target.classList.contains("mddtp-picker__cell")
      ) {
  ...
  _dragDial() {
    ...
    const onDragEnd = function () {
      const minuteViewChildren =
        me._sDialog.minuteView.getElementsByTagName("div");
      const sMinute = "mddtp-minute__selected";
      const selectedMinute = document.getElementById(sMinute);
      const cOffset = circle.getBoundingClientRect();
      fakeNeedle.style.left = `${cOffset.left - hOffset.left}px`;
